...
Правила по именам и фамилиям людей — соблюдай строго:
- Если имя или фамилия встречается в названии ролика — используй написание из
  названия как КАНОНИЧЕСКОЕ во всём ответе, включая теги.
- Несклоняемые фамилии (на -е, -и, -о, -у: Гаазе, Кюри, Шевченко, Дурново) НЕ
  склоняй и НЕ «восстанавливай» из косвенной формы именительный падеж:
  «с Константином Гаазе» — это фамилия «Гаазе», а не «Гааз».
- Если фамилия в транскрипте распознана неуверенно (Whisper мог ослышаться),
  а в названии ролика есть похожее имя — доверяй названию.
""".strip()
...
Схема:
{{
  "overview": "Executive summary ролика. Начни с ГЛАВНОЙ МЫСЛИ или ключевого вывода одной чёткой формулировкой по сути. Затем — необходимый контекст: о чём ролик, какой фокус, кому и зачем это релевантно. Объём: 3–5 предложений, компактно и содержательно. Разбей текст на 2–4 коротких абзаца, разделённых пустой строкой; первый абзац — главная мысль. Запрещено начинать с мета-фраз 'Ролик обсуждает…', 'Автор рассказывает о…', 'В видео говорится, что…', 'Видео посвящено…'. Сразу содержательный тезис.",
  "chapters": [
    {{
      "title": "Короткий содержательный заголовок тезиса (не таймкод, не 'Часть 1', не 'Введение')",
      "notes": "ПОДРОБНАЯ расшифровка тезиса без искусственных ограничений по длине. Включи саму идею, аргументацию автора, конкретные примеры, цифры, имена, цитаты, кейсы, нюансы, ограничения, контраргументы и практические выводы, если они есть. Пиши связным русским текстом; при необходимости — несколько абзацев, отделённых пустой строкой. Приоритет — ПОЛНОТА, а не краткость: не выбрасывай важное ради экономии символов. Но и не растекайся: каждое предложение с нагрузкой, без воды и повторов."
    }}
  ],
  "tags": {{
...
Схема:
{{
  "overview": "Executive summary ролика. Начни с ГЛАВНОЙ МЫСЛИ или ключевого вывода одной чёткой формулировкой по сути. Затем — необходимый контекст: о чём ролик, какой фокус, кому и зачем это релевантно. Объём: 3–5 предложений, компактно и содержательно. Разбей текст на 2–4 коротких абзаца, разделённых пустой строкой; первый абзац — главная мысль. Запрещено начинать с мета-фраз 'Ролик обсуждает…', 'Автор рассказывает о…', 'В видео говорится, что…', 'Видео посвящено…'.",
  "chapters": [
    {{
      "title": "Короткий содержательный заголовок тезиса (не таймкод, не 'Фрагмент 1', не 'Введение')",
      "notes": "ПОДРОБНАЯ расшифровка тезиса без искусственных ограничений по длине. Включи саму идею, аргументацию, конкретные примеры, цифры, имена, цитаты, кейсы, нюансы, ограничения, контраргументы и практические выводы. Пиши связным русским текстом; при необходимости несколько абзацев, отделённых пустой строкой. Приоритет — ПОЛНОТА, а не краткость."
    }}
  ],
  "tags": {{
...
Схема:
{{
  "overview": "Executive summary ролика — главная мысль + короткий контекст, 3–5 предложений. Разбей текст на 2–4 коротких абзаца, разделённых пустой строкой; первый абзац — главная мысль.",
  "chapters": [
    {{
...
class Summarizer:
    ...
    def _fix_literal_newlines(text: str) -> str:
        """Некоторые модели пере-экранируют переводы строк в JSON (пишут
        двойной бэкслеш), и после json.loads в тексте остаются ЛИТЕРАЛЬНЫЕ
        символы «\\n» вместо переводов строк (боевой кейс CqiM9HUHMO8,
        2026-08-28). Легитимный бэкслеш-n как СОДЕРЖИМОЕ саммари ролика
        практически невозможен — заменяем безусловно."""
        return text.replace('\\n', '\n')
    ...
    def _parse_summary(self, raw: str) -> Summary:
        ...
        chapters = [
            Chapter(
                ...
                notes=self._fix_literal_newlines(str(item.get("notes", "")).strip()),
            )
        ...
        return Summary(
            overview=self._fix_literal_newlines(str(data.get("overview", "")).strip() or raw.strip()),
            key_points=[],
            chapters=chapters,
            raw_text=raw,
            tags=tags,
