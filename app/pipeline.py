...
async def _maybe_retry_exhaustion_failure(
    ...
    Известное ограничение: статус-сообщение — один слот на чат
    (``services.summary_status_messages[chat_id]``), а окно ретрая тут
    долгое (до часа). Если за это время в том же чате стартует другой job,
    он может занять/перезаписать тот же слот, и наш retry-статус потеряется
    (эдит уйдёт либо в чужое сообщение, либо не туда). Это не хуже, чем
    сегодняшнее поведение любого другого best-effort статуса (Q6) — сам
    ретрай (deferred-строка в БД, run_after) от этого не страдает, теряется
    только промежуточная UI-плашка.
    """
