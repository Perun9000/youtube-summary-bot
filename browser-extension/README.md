...
с deep-link'ом `telegram.me/YouTube_Sum_mary_bot?start=<video_id>` — бот видит
...
браузер уходит на страницу-прослойку telegram.me, затем открывается desktop-клиент
...
- Токен пуст → поведение как раньше: сразу открывается deep-link
  `telegram.me/...?start=<id>`, никаких fetch-запросов не делается.
...
- На клик: открывает `https://telegram.me/YouTube_Sum_mary_bot?start=<id>` в новой
